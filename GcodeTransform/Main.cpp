...
Vector2 bedOffset(00, 00); // your origin is (0,0), the center of the bed is?
...
// matrix
Mat<double> xyzMatrix;
Mat<double> rotationXMatrix;
Mat<double> rotationYMatrix;
...
int main() {
	...
	rotationXMatrix << 1 << 0 << 0 << endr
		<< 0 << cos(rotX*M_PI / 180) << -sin(rotX*M_PI / 180) << endr
		<< 0 << sin(rotX*M_PI / 180) << cos(rotX*M_PI / 180) << endr;

	rotationYMatrix << cos(rotY*M_PI / 180) << 0 << sin(rotY*M_PI / 180) << endr
		<< 0 << 1 << 0 << endr
		<< -sin(rotY*M_PI / 180) << 0 << cos(rotY*M_PI / 180) << endr;

	cout << "Doing the matrix math.\n";
	xyzMatrix.print(cout);
	/*
	...
	for (int i = 0; i < sizeI; i++) {
		...
		long double finalY = xyzArray[i][0] - nozzleCompensation.y + bedOffset.y + partOffset.y;
		long double finalZ = xyzArray[i][0] - nozzleCompensation.z + partOffset.z;
